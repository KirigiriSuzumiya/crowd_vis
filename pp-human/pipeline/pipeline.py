...
from openvino_infer.openvino_mot_infer import OpenvineDetector
...
from python.visualize import visualize_box_mask, visualize_attr, visualize_pose, visualize_action, visualize_vehicleplate, visualize_mask
...
class PipePredictor(object):
        ...
        if not is_video:
            ...
            if self.with_mot or self.modebase["idbased"] or self.modebase[
                ...
                if run_mode == "openvino":
                    ...
                    if not files_list == ['infer_cfg.yml', 'model.onnx']:
                        raise "mot model dir error"
                    self.mot_predictor = OpenvineDetector(
        ...
        while (1):
            ...
            if self.modebase["idbased"] or self.modebase["skeletonbased"]:
                ...
                if len(mot_res['boxes']) == 0:
                    ...
                    if self.cfg['visual']:
                        ...
                        if self.file_name is None:  # use camera_id
                            temp1, temp2 = cv2.imencode('.jpeg', im)
                            print("frame_info:", temp2.tobytes())
                ...
                if self.mask:
                    ...
                    mask_result = mask_detector.face_detection(images=[frame])
                    self.pipeline_res.update(mask_result[0], 'mask')
                if self.with_vehicleplate:
                    if frame_id > self.warmup_frame:
            ...
            if self.cfg['visual']:
                ...
                fp = open('frame' + str(self.show_id) + '.txt', 'wb')
                fp.write(temp2.tobytes())
                fp.close()
                fp = open('records' + str(self.show_id) +'.txt', 'w')
                ajax_data = records[-1]
                if self.mask:
                    ajax_data = [ajax_data, len(mot_res['boxes']), mask_result]
                else:
    ...
    def visualize_video(self,
        ...
        mask_res = result.get('mask')
        if mask_res is not None:
            image = visualize_mask(image, mask_res)
        ...
        if video_action_res is not None:
            ...
            try:
                if video_action_score and video_action_score>0.6:
                    requests.get("http://127.0.0.1:8000/warn/fight")
            except:
                pass
        ...
        if skeleton_action_res is not None:
            ...
            print("warn-falling")
            try:
                requests.get("http://127.0.0.1:8000/warn/falling")
            except:
                pass
        ...
        if det_action_res is not None:
            ...
            print("warn-smoking")
            try:
                requests.get("http://127.0.0.1:8000/warn/smoking")
            except:
                pass
